...
namespace PRACTIC
{
    public partial class Form6 : Form
    {
        ...
        {
            dataGridView1.Columns.Add("ID_Defendant", "ID");
            dataGridView1.Columns.Add("FIO", "ФИО ответчик");
        ...
        private void RefData(DataGridView dgv)
        {
            ...
            string queru = $"select * from Defendant";
        ...
        private void Deleted()
        {
            try
            {
                ...
                string sql = "delete from Defendant where ID_Defandant =" + id;
                ExecSQL(sql);
            ...
            try
            {
                ...
                for (int index = 0; index < dataGridView1.Rows.Count; index++)
                {
                    ...
                    if (row == RowState.Modifind)
                    {
                        ...
                        var chenQ = $"update Defendant set FIO = '{FIO}',Passport = '{Passport}',PlaceOfLiving = '{PlaceOfLiving}', Phone = '{Phone}' where ID_Defandant = '{ID}'";
        ...
        private void NewFild()
        {
            try
            {
                ...
                var PlaceOfLiving = textBox5.Text;
                var Phone = textBox6.Text;
                var addQ = $"insert into Defendant (FIO,Passport,PlaceOfLiving, Phone) values ('{FIO}', '{Passport}','{PlaceOfLiving}','{Phone}')";
        ...
        {
            dgv.Rows.Clear();
            string seard = $"select * from Defendant where concat(ID_Defendant, FIO,Passport,PlaceOfLiving, Phone) like '%" + textBox4.Text + "%' ";
