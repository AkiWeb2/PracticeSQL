...
namespace PRACTIC
{
    public partial class Form4 : Form
    {
        ...
        {
            dataGridView1.Columns.Add("ID_Investigator", "ID");
            dataGridView1.Columns.Add("FIO", "ФИО следователя");
        ...
        private void RefData(DataGridView dgv)
        {
            ...
            string queru = $"select * from Investigator";
        ...
        private void Deleted()
        {
            try
            {
                ...
                string sql = "delete from Investigator where ID_Investigator =" + id;
                ExecSQL(sql);
            ...
            try
            {
                ...
                for (int index = 0; index < dataGridView1.Rows.Count; index++)
                {
                    ...
                    if (row == RowState.Modifind)
                    {
                        ...
                        var chenQ = $"update Investigator set FIO = '{FIO}', Phone = '{Phone}' where ID_Investigator = '{ID}'";
        ...
        private void NewFild()
        {
            try
            {
                ...
                var FIO = textBox2.Text;
                var Phone = textBox3.Text;
                var addQ = $"insert into Investigator (FIO, Phone) values ('{FIO}', '{Phone}')";
        ...
        {
            dgv.Rows.Clear();
            string seard = $"select * from Investigator where concat(ID_Investigator, FIO, Phone) like '%" + textBox4.Text + "%' ";
